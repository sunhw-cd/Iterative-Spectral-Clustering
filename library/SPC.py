...
def local_scaled_affinity(x, y, k=3, method='fcg'):
    """
    ...
    if method=='knn':
        # Find k-nearest neighbors for input data points
        nbrs = NearestNeighbors(n_neighbors=k, algorithm='brute', metric='euclidean', n_jobs=-1).fit(y)
        affinity = nbrs.kneighbors_graph(mode='connectivity')
        #affinity = affinity + np.eye(affinity.shape[0])

 
    return affinity

def Laplacian_matrix(affinity, mode="sym"):
    """
