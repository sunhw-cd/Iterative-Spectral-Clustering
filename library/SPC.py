...
def local_scaled_affinity(x, y, k=3):
    ...
    affinity=affinity/np.expand_dims(np.sum(affinity,axis=-1),axis=-1)
 
    return affinity

def Laplacian_matrix(affinity):
    """
    ...
    Returns:
    rw_L_matrix : Random walk Laplacian matrix
    """
     
    A_matrix = affinity  # Affinity matrix

    # Compute degree matrix (sum of affinities for each row)
    D_matrix = np.sum(A_matrix, axis=-1) * np.eye(A_matrix.shape[0])

    # Compute unnormalized Laplacian matrix
    unnorm_L_matrix = -A_matrix + D_matrix
    
    # Compute random walk Laplacian matrix
    rw_L_matrix = np.dot(np.linalg.inv(D_matrix), unnorm_L_matrix)
